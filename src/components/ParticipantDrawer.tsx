...
export function ParticipantDrawer({
  ...
  return (
    <AnimatePresence>
      {participant ? (
        <motion.div
          className="fixed inset-0 z-50 flex justify-end bg-black/30 p-3 backdrop-blur-[2px]"
          initial={{ opacity: 0 }}
          animate={{ opacity: 1 }}
          exit={{ opacity: 0 }}
          onClick={onClose}
        >
          <motion.aside
            className="surface flex h-full w-full max-w-md flex-col overflow-hidden !rounded-2xl"
            ...
            role="dialog"
            aria-modal="true"
            aria-label={`Detalle de ${participant.name}`}
          >
